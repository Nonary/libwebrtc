...
# In this repository the wrapper sources live at the repo root, so RootDir is
# the libwebrtc checkout itself (scripts/..).
$scriptRoot = Resolve-Path (Join-Path $PSScriptRoot "..")
$RootDir = Resolve-Value -Value $RootDir -CacheKey "" -EnvKey "LIBWEBRTC_ROOT_DIR"
if (-not $RootDir) {
  $RootDir = $scriptRoot
}
...
# multi-hour build is not coupled to any single checkout.
# Override priority: LIBWEBRTC_DEPS_DIR env var > WEBRTC_BUILD_DIR/WEBRTC_OUT_DIR
# (legacy, unchanged) > %LOCALAPPDATA%\libwebrtc\deps default.
$DepsRoot = $env:LIBWEBRTC_DEPS_DIR
if (-not $DepsRoot) {
  if ($env:LOCALAPPDATA) {
    $DepsRoot = Join-Path $env:LOCALAPPDATA "libwebrtc\deps"
  } else {
    $DepsRoot = Join-Path $RootDir ".deps"
  }
}
...
# The wrapper sources are this repository itself. Copy only tracked top-level
# entries so .git, scripts, workflows, and local build/deps junk stay out of
# the GN tree.
...
New-Item -ItemType Directory -Path $destLibWebrtc | Out-Null
$wrapperEntries = & $GitExe -C $RootDir ls-tree --name-only HEAD
if ($LASTEXITCODE -ne 0 -or -not $wrapperEntries) {
  throw "Unable to enumerate wrapper sources from git tree at $RootDir"
}
foreach ($entry in @($wrapperEntries)) {
  if ($entry -in @("scripts", ".github", ".gitignore", ".gitattributes")) {
    continue
  }
  Copy-Item -Recurse -Force (Join-Path $RootDir $entry) (Join-Path $destLibWebrtc $entry)
}
