...
$releaseTitle = "libwebrtc $tag (Windows x64)"
$releaseExists = $false
...
if (-not $releaseExists) {
  ...
  if ($NotesPath -and (Test-Path -LiteralPath $NotesPath)) {
    ...
    $createArgs += @("--notes", "Prebuilt Windows x64 libwebrtc artifacts.")
  }
  if ($env:GITHUB_SHA) {
...
# Also attach the pinned manifest so consumers can refresh their committed
# manifest (and its SHA256) straight from the release.
Write-Step "Uploading $(Split-Path -Leaf $ManifestPath)"
& $gh.Source release upload $tag $ManifestPath --repo $repo --clobber
if ($LASTEXITCODE -ne 0) {
  throw "Failed to upload WebRTC release manifest '$(Split-Path -Leaf $ManifestPath)'."
}
